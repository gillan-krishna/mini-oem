...
def preprocess_fn(data, to_tensor, preprocess_input):
...
class OpenEarthMapDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, img_list: list, n_classes: int = 9, testing=False, augm=None, encoder=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.encoder is None:
            ...
            return data["image"], data["mask"], self.fn_imgs[idx]
                ...
                preprocess_input = smp.encoders.get_preprocessing_fn(encoder_name=self.encoder)
                data = preprocess_fn(
                data, self.to_tensor, preprocess_input
            )
